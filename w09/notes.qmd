...
|:- |:- |:- |:- |:-:|
| **Lecture** | 12:30pm | 1:00pm | <a href="#markov-chains">Law of Large Numbers &rarr;</a> | <a id="rec-link-w06-2" href="../recordings/recording-w06s02-2.html" target="_blank" class="disabled"><i class="bi bi-film"></i></a> |
| | <span class='sec-p55'>1:00pm</span> | <span class='sec-p70'>1:30pm</span> | <a href="#pagerank">Central Limit Theorem &rarr;</a> | <a id="rec-link-w06-3" href="../recordings/recording-w06s02-3.html" target="_blank" class="disabled"><i class="bi bi-film"></i></a> |
| | <span class='sec-p55'>1:30pm</span> | <span class='sec-p70'>2:00pm</span> | <a href="#matrix-magic">Maximum Likelihood Estimation &rarr;</a> | <a id="rec-link-w06-3" href="../recordings/recording-w06s02-3.html" target="_blank" class="disabled"><i class="bi bi-film"></i></a> |
| | <span class='sec-p55'>1:30pm</span> | <span class='sec-p70'>2:00pm</span> | <a href="#matrix-magic">Method of Moments &rarr;</a> | <a id="rec-link-w06-3" href="../recordings/recording-w06s02-3.html" target="_blank" class="disabled"><i class="bi bi-film"></i></a> |
| **Break!** | <span class='sec-p80'>1:50pm</span> | <span class='sec-p90'>2:00pm</span> | | |
| | <span class='sec-p55'>2:00pm</span> | <span class='sec-p70'>2:20pm</span> | <a href="#matrix-magic">Intro to Bias-Variance Tradeoff &rarr;</a> | <a id="rec-link-w06-3" href="../recordings/recording-w06s02-3.html" target="_blank" class="disabled"><i class="bi bi-film"></i></a> |
| **Lab** | <span class='sec-p100'>2:20pm</span> | <span class='sec-p130'>2:40pm</span> | <a href="#lab-demonstrations">Lab &rarr;</a> | <a id="rec-link-w06-5" href="../recordings/recording-w06s02-5.html" target="_blank" class="disabled"><i class="bi bi-film"></i></a> |
| | <span class='sec-p130'>2:40pm</span> | <span class='sec-end'>3:00pm</span> | Student Presentation | |

: {tbl-colwidths="[14,12,12,50,12]"}

## Statistical Inference {data-name="Samples vs. Populations"}
...
## Zooming in on Euclid {.small-captions .crunch-figures}
...
::: {layout-ncol=2}
![Source: <a href="https://emilypiercemath221.weebly.com/math-221-blog/triangles-everywhere" target="_blank">Emily Pierce</a>](/assets/img/mountain_triangle.jpg)

![Source: <a href="https://commons.wikimedia.org/wiki/File:All_Gizah_Pyramids.jpg" target="_blank">Wikimedia</a>](/assets/img/pyramids_giza_crop.jpg)
:::

::: {#arrowhead}

```{=html}
<iframe title="Badin Crude Triangular (312a402)" width="100%" frameborder="0" allowfullscreen mozallowfullscreen="true" webkitallowfullscreen="true" allow="fullscreen; xr-spatial-tracking" xr-spatial-tracking execution-while-out-of-viewport execution-while-not-rendered src="https://sketchfab.com/models/fcb7ae394454455da7912369a389e426/embed"></iframe>
```
...
![13th-Century Arabic *Elements*, from <a href="http://www.sothebys.com/en/auctions/ecatalogue/2018/arts-of-the-islamic-world-l18220/lot.30.html" target="_blank">Sotheby's</a>](/assets/img/euclid_crop.jpg){fig-align="center" height=50%}
...
# Law of Large Numbers {data-name="Law of Large Numbers"}

## Stability out of Randomness

* $X$ = result of coin flip
* Remembering that $X$ is a **random variable**, so it maps **outcomes** to **numbers**: $X($<span class="svg-span"><img src="/assets/img/puerto_rico_adobe_express.svg" height="70px" style="vertical-align: bottom; transform: translate(0, 18px)"></span>$) = 0$, $X($<span class="svg-span"><img src="/assets/img/quarter_adobe_express.svg" height="70px" style="display: inline-flex; vertical-align: bottom; transform: translate(0, 18px)"></span>$) = 1$
...
## Central Limit Theorem {data-name="Central Limit Theorem"}
...
## When is $\mathcal{N}(0, 1)$ a "good" approximation? {.smalltitle}
...
## Maximum Likelihood Estimation {data-name="Maximum Likelihood"}
...
Given a choice of $\theta$, we can generate as many **simulated datasets** as we want (in this case, 10 datasets for each $\theta$ value), then compute **likelihood** of producing 8 heads, 2 tails
...
1. What are the individual $\mathcal{L}(x_i \mid p)$ terms?
2. How do we maximize the product $\mathcal{L}(x_1 \mid p) \cdots \mathcal{L}(x_n \mid p)$?
...
1. What are the individual $P(x_i \mid p)$ terms?
...
So, to **maximize** likelihood function, we need to find its **derivative**[^deriv], set it equal to 0, and solve:
...
[^deriv]: This is why we used math magic to make $P(x_i \mid \theta)$ differentiable, in the previous slide!
...
* If we had terms that were **added** rather than **multiplied**, we'd have a much easier time: $\frac{d}{d\theta}\left[ \mathcal{L}(x_1) + \mathcal{L}(x_2)\right] = \frac{d}{d\theta}P(x_1) + \frac{d}{d\theta} P(x_2)$[^linear]
* So, what math operation do we know that turns **multiplications** into **additions**?

[^linear]: We achieve this simplification because the derivative operator is **linear**: $\frac{d}{dx}\left[ f(x) + g(x) \right] = \frac{d}{dx}f(x) + \frac{d}{dx}g(x)$
...
Now that we know $\frac{d}{dp}\ell(x_i)$, we set our log-likelihood equation equal to zero to find the likelihood-maximizing $p$ value, $p^*$:
...
* Almost there! We know that $\expect{\mu^*} = \mu$, but what is $\Var{\mu^*}$? Remember that $\Var{aX} = a^2\Var{X}$!
...
## Handwavy Intuition {.crunch-math .smaller}
...
## 21st-Century Solution {.smaller .crunch-math .nobotmargin}
...
## {.smaller .crunch-math .nobotmargin}
...
## Appendix 1: Derivation of $\expect{\mu_*^2}$ {.smaller .crunch-math}
...
## Appendix 2: Derivation of $\expect{\mu^*X_i}$ {.smaller}
...
## Appendix 3: Derivation of $\expect{\sigma^2_*}$ {.smaller .crunch-math}
