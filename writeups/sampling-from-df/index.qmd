...
A few students have run into issues when trying to use the `sample()` function, from base-R, to sample from a full `data.frame` or `tibble`. In this writeup I'll argue that this is a case where using a function from the `tidyverse` will make your life much easier, but I will also show how to do this sampling using only base-R functions.
