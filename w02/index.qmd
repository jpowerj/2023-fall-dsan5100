...
## Example: Rolling Two Dice {.smaller}

* **Logic**: We can define 4 predicates:
  * $p_1$ = "First roll is $H$", $q_1$ = "First roll is $T$"
  * $p_2$ = "Second roll is $H$", $q_2$ = "Second roll is $T$"
* **Logical formulas**:
  * $f_1 = p_1 \wedge q_2$: "First roll is $H$ and second roll is $T$"
  * $f_2 = p_1 \wedge q_2$: "First roll is $H$ or second roll is $T$"
  * $f_3 = \neg p_1$: "First roll is not $H$"
* **The issue?**: We don't know, until **after** the dice have been rolled, whether these are **true** or **false**!
...
## Rolling Two Dice: Logic $\rightarrow$ Probability {.smaller}

* Returning to the dice example: we can look at the predicates and see that they exhaust all possibilities, so that we can define a **sample space** $S = \{TT, TH, HT, HH\}$ of all possible **outcomes** of our dice-rolling experiment, noting that $|S| = 4$, so there are 4 possible outcomes.
...
| - | - | - |
| $p_1$ = "First roll is $H$" | $P_1 = \{HT, HH\}$ | $\Pr(P_1) = \frac{|P_1|}{|S|} = \frac{2}{4} = \frac{1}{2}$ |
| $q_1$ = "First roll is $T$" | $Q_1 = \{TT, TH\}$ | $\Pr(Q_1) = \frac{|Q_1|}{|S|} = \frac{2}{4} = \frac{1}{2}$
| $p_2$ = "Second roll is $H$" | $P_2 = \{TH, HH\}$ | $\Pr(P_2) = \frac{|P_2|}{|S|} = \frac{2}{4} = \frac{1}{2}$ |
| $q_2$ = "Second roll is $T$" | $Q_2 = \{TT, HT\}$ | $\Pr(Q_2) = \frac{|Q_2|}{|S|} = \frac{2}{4} = \frac{1}{2}$
...
## Using "Rules" of Probability {.smaller}
...
## Converting to Probability Theory {.smaller}

* So, using the same principles we used in our dice rolling examples, we can consider **events** $P$ and $Q$, and get the following "translation" of DeMorgan's Laws:
...
$$
\begin{align*}
S = \{ &\text{result of dice roll was 1}, \\
...
* Each value $v$ that a random variable $X$ can take on gives rise to an **event** $X = v$: the **event** that the random variable $X$ takes on value $v$.
* Since $X = v$ is an **event**, we can compute its probability $\Pr(X = v)$!

## Doing Math with Events {.smaller}

* We can also (think of our analogy with logic, but this time think of algebra) **do math** with the random variables! We can think of events related to values $X$ can take on, and compute their probabilities
$$
\begin{align*}
